	...
	this.save_playlist = function() {
		...
		if(Ext.getCmp("playlist_selectionID").getValue() > 0) {
			...
			if(pl.selectedIndex >= 0) {
				...
				else echo "0"; ?> || (userID == <?php echo USERID ?> && <?php 
			...
			Ext.getCmp("playlistID").getEl().mask("Saving");
			Ext.Ajax.request({
	...
	this.play_song = function() {
		if(this.inited == 0) {
			...
			this.stop_playlist = false;
				...
				url = "functions/streamnet.php?ID=" + data.ID; 
			...
			url = "functions/stream.php?ID=" + data.ID; 
				<?php 
				} 
				?>
			...
			if(data.albumID && album) document.getElementById("info_album").innerHTML = "<a href=\"javascript: void(0)\" onClick=\"filter_setting('album'," + data.albumID + ");\">" + album + "</a>";
			...
			else document.title = "Unknown Song";
			...
			this.mySound = soundManager.createSound({
				...
				whileloading: function() {
					max_width = 178;
					document.getElementById("loadingbar").style.width = Math.round(this.bytesLoaded / this.bytesTotal * max_width) + "px";
				}
