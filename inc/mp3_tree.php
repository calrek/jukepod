...
class mp3_tree {
  ...
  function __construct( $id ) {
    ...
    if( ! $id) {
      ...
      } //$this->db->result($result, 0, "full_path")
      else {
        $this->root = MP3_PATH;
        $this->rootID = 0;
        $this->import_path = "";
      ...
      while(false !== ($entry = readdir($dh))) {
        if($entry != '.' and $entry != '..') {
          ...
          if(is_dir($path) AND $this->options["SCAN_METHOD"] != "SCAN_ONLY_FILES_IN_CURRENT_DIR") {
            ...
            } //$new OR $this->options["SCAN_METHOD"] != "SCAN_ONLY_NEW"
          } //is_dir($path) AND $this->options["SCAN_METHOD"] != "SCAN_ONLY_FILES_IN_CURRENT_DIR"
